# BanglaHost (Windows) - build distributables.
#
...
#                            # BanglaHost fetches/extracts via Windows' SIGNED curl.exe + tar.exe
#                            # (so banglahost.exe is never the process that drops executables).
...
dotnet publish src/BanglaHost.App/BanglaHost.App.csproj -c $Configuration -r $Rid --self-contained true -p:WindowsAppSDKSelfContained=true -o $publish
dotnet publish src/BanglaHost.Cli/BanglaHost.Cli.csproj -c $Configuration -r $Rid --self-contained true -o $publish
dotnet publish src/BanglaHost.Elevate/BanglaHost.Elevate.csproj -c $Configuration -r $Rid --self-contained true -o $publish
...
if ($Bundle) {
    ...
    # Use the freshly-published CLI to populate payload\bin via BANGLAHOST_HOME.
    $cli = Join-Path $publish "banglahost.exe"
    $env:BANGLAHOST_HOME = $payload
    & $cli init | Out-Null
    ...
    Remove-Item Env:\BANGLAHOST_HOME
...
& $iscc @bundleDef "installer\banglahost.iss"

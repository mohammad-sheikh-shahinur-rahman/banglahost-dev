# Security policy & threat model — BanglaHost for Windows

BanglaHost is a **local development** web stack. It is meant to run on a developer's own
...
**Everything binds to loopback (`127.0.0.1`) only.** No BanglaHost-managed service listens on
...
users with passwords are available (`banglahost db create <name> <password>`).
...
- **The elevated helper** (`banglahost-elevate.exe`, used only for hosts-file edits + the mkcert
...
**remote/LAN** actor to reach a BanglaHost service. Those are taken seriously and fixed quickly.

...
BanglaHost for Windows is **open-source and unsigned** (no paid code-signing certificate).
...
BanglaHost is a *local web-server manager*: it downloads server binaries (nginx, PHP, MySQL,
...
executables** as a generic "dropper" (e.g. `Win64/Agent_AGen`). BanglaHost does exactly that —
legitimately — so it can trip that heuristic and the app may get deleted/quarantined.

BanglaHost already minimizes this: every download is performed by Windows' own **signed**
...
1. **Restore the app if it was already quarantined.**
   - ESET: **Tools → Quarantine** → select `BanglaHost.App.exe` → **Restore**. (Or just re-run the installer.)
2. **Add exclusions** so it won't be flagged again. Exclude these two folders:
   - `C:\Program Files\BanglaHost`
   - `%LOCALAPPDATA%\BanglaHost`  (i.e. `C:\Users\<you>\AppData\Local\BanglaHost`)
   ...
   *Manage settings* → *Exclusions* → **Add an exclusion → Folder** → pick each folder.
   ...
   ```powershell
   Add-MpPreference -ExclusionPath "$env:ProgramFiles\BanglaHost", "$env:LOCALAPPDATA\BanglaHost"
   ```
3. Launch BanglaHost and install the services you need — it will work normally.

...
/// <summary>
/// The brains — the C# analog of the mac <c>engine/banglahost</c> script. The WinUI
...
public sealed class Engine
{
    ...
    private static void NeedInit()
    {
        if (!Directory.Exists(Paths.Config))
            throw new BhException("not initialized — run: banglahost init");
    }
    ...
    public void Install(string tool)
    {
        ...
        if (string.IsNullOrEmpty(tool))
            throw new BhException("usage: banglahost install <all|nginx|apache|php@8.4|mariadb|redis|memcached|mkcert|mailpit|fnm|cloudflared>");
        var cfg = Config.Load();
    ...
    public void Start(string svc)
    {
        ...
        if (svc is "all" or "")
        {
            ...
            foreach (var v in SitePhpVersions(cfg))
                ...
                else Warn($"php {v} not installed — banglahost install php@{v}");
        ...
        if (svc == "mailpit") { if (MailpitServer.Start()) Ok($"mailpit on UI :{MailpitServer.UiPort} / SMTP :{MailpitServer.SmtpPort}"); else throw new BhException("mailpit not installed — banglahost mailpit"); return; }
    ...
    /// <summary>A friendly branded landing page written as a site's index.php when it has no own
    /// index yet (non-WordPress sites). Verbatim string with single-quoted HTML so there are no
    /// double-quotes to escape; PHP fills in host/PHP/server/folder at runtime.</summary>
    private const string DefaultIndexPhp = @"<?php
...
";
    ...
    public void SiteAdd(string name, string php = "", string? root = null, string server = "", string type = "others")
    {
        ...
        // Allow dots for multi-label local domains (e.g. my.banglahost -> my.banglahost.test),
        ...
        else Warn($"php {version} not installed — banglahost install php@{version} (site will 502 until then)");
    ...
    public void Status()
    {
        ...
        Hdr("Sites");
        if (snap.Sites.Count == 0) Info("no sites yet — banglahost site add <name>");
    ...
    private void EnsureMkcertCa(string mkc)
    {
        ...
        if (Elevation.Run("mkcert-install")) Ok("mkcert CA installed (browsers will trust BanglaHost certs)");
    ...
    public void ConfigSet(string key, string val)
    {
        ...
        if (key is "http_port" or "https_port" or "tld")
        {
            ...
            if (key == "tld") Warn($"TLD changed — re-issue HTTPS per site: banglahost secure <name>.{val}");
        }
    }
    ...
    public void Db(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            case "" or "list":
                ...
                if (dbs.Count == 0) Info("none — start the server (banglahost start mariadb) then create one");
    ...
    public void Pg(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            case "" or "list":
                ...
                if (dbs.Count == 0) Info("none — start the server (banglahost start postgresql) then create one");
                foreach (var d in dbs) Ok(d);
                break;
            case "create":
    ...
    public void PySiteList()
    {
        ...
        if (!any) Info("none — banglahost pysite add <name> --dir … --port … [--cmd …] [--venv yes]");
    }
    ...
    public void Tunnel(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            ...
            case "" or "list" or "status":
                ...
                if (!any) Info("none running — banglahost tunnel start <site>");
                break;
    ...
    public void PhpMyAdmin()
    {
        ...
        if (!DbServer.Running()) Warn("database not running — start it so phpMyAdmin can log in: banglahost start mariadb");
    ...
    public void Mailpit()
    {
        ...
        if (BanglaHost.Core.MailpitServer.Start())
            Ok($"mailpit running — SMTP :{BanglaHost.Core.MailpitServer.SmtpPort}, UI :{BanglaHost.Core.MailpitServer.UiPort}");
        ...
        Ok($"site vhost: mailpit.conf (proxy → :{BanglaHost.Core.MailpitServer.UiPort})");
        if (Nginx.Running()) Nginx.Reload(cfg);
        else { var (ok, msg) = Nginx.Start(cfg); if (ok) Ok(msg); else Warn(msg); }
        EnsureHosts(domain);
        ...
        Info($"SMTP   : 127.0.0.1:{BanglaHost.Core.MailpitServer.SmtpPort}  — point php sendmail/SMTP here");
    }

...
/// <summary>
/// In-app updater — checks the GitHub releases for a newer BanglaHost-Setup.exe and
...
{
    private const string Repo = "mohammad-sheikh-shahinur-rahman/BanglaHost";
    ...
    public static async Task<Result> Check()
    {
        try
        {
            ...
            http.DefaultRequestHeaders.UserAgent.ParseAdd("BanglaHost-Updater");
    ...
    public static async Task DownloadAndRun(string url)
    {
        ...
        http.DefaultRequestHeaders.UserAgent.ParseAdd("BanglaHost-Updater");
        var dest = Path.Combine(Path.GetTempPath(), "BanglaHost-Setup-update.exe");
        await using (var s = await http.GetStreamAsync(url))
        await using (var f = File.Create(dest))
            await s.CopyToAsync(f);
        // Launch the installer, then fully exit BanglaHost (incl. the tray) so the running
        // BanglaHost.App.exe / Core.dll unlock and the installer can replace them without "couldn't

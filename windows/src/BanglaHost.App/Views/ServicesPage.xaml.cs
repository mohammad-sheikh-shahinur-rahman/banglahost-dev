...
public sealed partial class ServicesPage : Page
{
    ...
    private void AutoStar_Click(object sender, RoutedEventArgs e)
    {
        ...
        var cfg = Config.Load();
        if (tb.IsChecked == true) BanglaHost.Core.Services.Enable(key, cfg);
        else BanglaHost.Core.Services.Disable(key, cfg);
    }
    ...
    private async void Uninstall_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Title = "Uninstall", Content = $"Remove the {key} binaries from BanglaHost's bin folder?",
